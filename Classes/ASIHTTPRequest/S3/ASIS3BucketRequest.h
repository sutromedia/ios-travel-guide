...
@interface ASIS3BucketRequest : ASIS3Request {
	...
	NSString *__unsafe_unretained subResource;
	
	// Options for filtering GET requests
	// See http://docs.amazonwebservices.com/AmazonS3/2006-03-01/index.html?RESTBucketGET.html
	NSString *__unsafe_unretained prefix;
	NSString *__unsafe_unretained marker;
	int maxResultCount;
	NSString *__unsafe_unretained delimiter;
	
	// Internally used while parsing the response
	ASIS3BucketObject *__unsafe_unretained currentObject;
	
	// Returns an array of ASIS3BucketObjects created from the XML response
	NSMutableArray *__unsafe_unretained objects;	
	
	// Will be populated with a list of 'folders' when a delimiter is set
	NSMutableArray *__unsafe_unretained commonPrefixes;
...
@property ( unsafe_unretained, nonatomic) NSString *bucket;
@property ( unsafe_unretained, nonatomic) NSString *subResource;
@property ( unsafe_unretained, nonatomic) NSString *prefix;
@property ( unsafe_unretained, nonatomic) NSString *marker;
@property (assign, nonatomic) int maxResultCount;
@property ( unsafe_unretained, nonatomic) NSString *delimiter;
@property ( unsafe_unretained, readonly) NSMutableArray *objects;
@property ( unsafe_unretained, readonly) NSMutableArray *commonPrefixes;
