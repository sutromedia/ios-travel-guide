...
static NSTimeInterval defaultTimeOutSeconds = 20; //TF updated from 10
...
- (void)requestStarted
{
    ...
    //TF Added try and catch to hopefully prevent intermittent crashing bug
    @try {
        if (delegate && [delegate respondsToSelector:didStartSelector]) { //TF 052212 Bad accesss crash here
            [delegate performSelector:didStartSelector withObject:self];
        }
    }
    @catch (NSException *exception) {
        NSLog(@"?**** WARNING: ASIHTTPREQUEST.requestStarted: Caught exception! ****");
		return;
    }
    @finally {
        NSLog(@"");
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		@catch (NSException *exception) {
			NSLog(@" **** ASIHTTPREQUEST.handleBytesAvailable: Exception Caught ****** ");
			return;
		}
		@finally {
			NSLog(@"");
