...
@interface ASIDownloadCache : NSObject <ASICacheDelegate> {
	...
	NSString *__unsafe_unretained storagePath;
	
	// Mediates access to the cache
	NSRecursiveLock *__unsafe_unretained accessLock;
...
@property ( unsafe_unretained, nonatomic) NSString *storagePath;
@property  (unsafe_unretained) NSRecursiveLock *accessLock;
